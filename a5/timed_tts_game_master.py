...
def runGame():
    ...
    while not FINISHED:
        ...
        CURRENT_PLAYER = who
        if WhitesTurn:
            playerResult = timeout(player1.take_turn, args=(currentState, currentRemark, TIME_PER_MOVE), kwargs={},
                                   timeout_duration=TIME_PER_MOVE, default=(None, "I give up!"));
            name = player1.moniker()
            WhitesTurn = False
        else:
            playerResult = timeout(player2.take_turn, args=(currentState, currentRemark, TIME_PER_MOVE), kwargs={},
                                   timeout_duration=TIME_PER_MOVE, default=(None, "I give up!"));
...
def timeout(func, args=(), kwargs={}, timeout_duration=1, default=None):
    ...
    if pt.isAlive():
        ...
        if USE_HTML: gameToHTML.reportResult(
            "Player " + CURRENT_PLAYER + " took too long (%0.4f seconds) and thus loses." % diff)
        if USE_HTML: gameToHTML.endHTML()
        exit()
    else:
