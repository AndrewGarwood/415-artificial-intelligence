...
import awg1024_Q_Learn as Q_Learn
...
NOISE_VAR = None;
NGOALS_VAR = None;
GOLDEN_PATH_VAR = None;
R_VAR = None;
G_VAR = None
DISPLAY_VALS_VAR = None;
Q_VAR = None;
EPSILON_VAR = None;
EXPL_VAR = None
VI_POLICY_VAR = None;
...
def create_TK_Canvas():
    ...
    TK_Canvas.create_rectangle(0, HEIGHT * 0.3 - 70, WIDTH, HEIGHT,
...
def enable_policy_extraction(tf):
    if tf:
        VI_MENU.entryconfig("Show Policy from VI", state="normal")
    else:
        VI_MENU.entryconfig("Show Policy from VI", state="disabled")


def enable_QL_policy_item(tf):
    if tf:
        QLEARN_MENU.entryconfig("Show Policy from QL", state="normal")
    else:
        QLEARN_MENU.entryconfig("Show Policy from QL", state="disabled")
...
VALUE_FONT = None
MDP_command = None
GOLDEN_PATH_EDGES = None;
SILVER_PATH_EDGES = None
...
def display_values(V):
    ...
    for s, v in V.items():
        try:
            ...
            pass  # Terminal_state has no coordinates.
...
def show_policy(pi, policy_number=0, use_alt_segments=False, color="brown"):
    ...
    for s in pi.keys():
        try:
            ...
            PI_LINE_BUFS[policy_number].append(
                TK_Canvas.create_line(xc + dx0, yc + dy0, xc + dx1, yc + dy1, arrow=tk.LAST, fill=color))
        except:
...
def update_q_value(s, a, value):
    ...
    try:
        ...
        print("No match in update_q_value for key " + str((s, a)))
...
def TOH_state_vis(s):
    ...
    SV_RECTS.append(
        TK_Canvas.create_rectangle(XCENTER - BASE_WIDTH / 2, YBASE, XCENTER + BASE_WIDTH / 2, YBASE - BASE_HEIGHT,
                                   fill="black"))
    ...
    for p in ['peg1', 'peg2', 'peg3']:
        # Draw a peg:
        SV_RECTS.append(TK_Canvas.create_rectangle(xpeg - PEG_RADIUS, YBASE - BASE_HEIGHT, xpeg + PEG_RADIUS,
                                                   YBASE - BASE_HEIGHT - peg_height, fill="brown"))
        # Draw the disks
