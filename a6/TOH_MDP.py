...
def T(s, a, sp):  # Here is the heart of the MDP: its transition model.
  ...
  '''
    ...
    if is_valid_goal_state(s):
        if a == "Exit" and sp == Terminal_state:
            return 1.0
        else:
            return 0.0
...
def MDP_command(cmd, param):
    ...
    if cmd == "ngoals":
        NGOALS = param
        if NGOALS == 2:
            SILVER_PATH = make_solution_path(path_type="silver")
        else:
            SILVER_PATH = []
...
def user_drives_agent_via_text_input():
    ...
    try:
        ...
        print("Unrecognized action.  Retry ...")
...
def make_solution_path(path_type="golden"):
    ...
    if parity:
        little_disk_ops = op_grp2;
        other_ops = op_grp1
    else:
        little_disk_ops = op_grp1;
        other_ops = op_grp2
